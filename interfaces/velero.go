...
type VeleroService interface {
	...
	GetDownloadRequests(ctx context.Context, namespace string) ([]velerov1.DownloadRequest, error)

	// DataUpload(v2) 객체 목록을 조회하여 백업 업로드 상태를 확인합니다.
	GetDataUploads(ctx context.Context, namespace string) ([]velerov2.DataUpload, error)

	// DataDownload(v2) 객체 목록을 조회하여 백업 다운로드 상태를 확인합니다.
	GetDataDownloads(ctx context.Context, namespace string) ([]velerov2.DataDownload, error)

	// 서버 상태 확인 요청(ServerStatusRequest) 목록을 조회합니다.
	GetServerStatusRequests(ctx context.Context, namespace string) ([]velerov1.ServerStatusRequest, error)

	// Src, Dest의 스토리지 클래스 목록을 조회하여 환경이 같은지 체크합니다.
	//ValidateStorageClasses(ctx context.Context) (models.ValidateStorageClassesResponse, error)
}
