...
// @Accept json
// @Produce json
// @Param request body github.com/taking/velero/internal/model.HelmConfigRequest true "헬름 연결에 필요한 값"
...
func (c *HelmController) IsChartInstalled(ctx echo.Context) error {
	...
	if err != nil {
		return response.RespondError(ctx, http.StatusInternalServerError, fmt.Sprintf("failed to check chart installation: %v", err))
	}

	return response.RespondSuccess(ctx, map[string]interface{}{
		"chart":     data,
		"installed": installed,
	})
}
...
// @Accept json
// @Produce json
// @Param request body model.HelmInstallChartRequest true "헬름 연결에 필요한 값"
// @Success 200 {object} model.SwaggerSuccessResponse "연결 성공"
// @Failure 400 {object} model.SwaggerErrorResponse "잘못된 요청"
// @Failure 500 {object} model.SwaggerErrorResponse "서버 내부 오류"
// @Failure 503 {object} model.SwaggerErrorResponse "서비스 이용 불가"
...
func (c *HelmController) InstallChart(ctx echo.Context) error {
	...
	if err := ctx.Bind(&req); err != nil {
		return response.RespondError(ctx, http.StatusBadRequest, "invalid request body")
	}

	// Helm 클라이언트 생성
	client, err := client.NewHelmClient(req.KubeConfig)
	if err != nil {
		return response.RespondError(ctx, http.StatusInternalServerError, err.Error())
	}
	...
	if err := client.InstallChart(req.ChartName, req.ChartPath, req.Values); err != nil {
		return response.RespondError(ctx, http.StatusInternalServerError, fmt.Sprintf("failed to install chart '%s': %v", req.ChartName, err))
	}

	return response.RespondStatus(ctx, "success", fmt.Sprintf("chart installed successfully: %s", req.ChartName))
}
