...
// HealthResponse 헬스체크 전용 응답 구조체
type HealthResponse struct {
	Status    string    `json:"status"`    // "healthy" 또는 "unhealthy"
	Service   string    `json:"service"`   // 서비스명 (예: "velero", "kubernetes", "minio")
	Message   string    `json:"message"`   // 상태 메시지
	Timestamp time.Time `json:"timestamp"` // 체크 시각
	Details   *struct { // 추가 상세 정보 (선택적)
